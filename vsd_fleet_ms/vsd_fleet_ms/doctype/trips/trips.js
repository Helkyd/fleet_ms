...
frappe.ui.form.on("Trips", {
	refresh: function (frm) {
			...
			frm.add_custom_button(
				...
				function () {
					...
					if (frm.doc.docstatus == 1) {
						frappe.db.set_value("Trips", frm.doc.name, {
							trip_completed: 1,
							trip_completed_date: frappe.datetime.nowdate(),
							trip_status: "Completed"
								...
								if (frm.doc.transporter_type == "In House") {
									console.log('76 - TRUCK +++++ ', truck);
									frappe.db
										.set_value("Truck", truck, {
								...
								frappe.msgprint("Error updating document.");
								console.error(error);
						});
		...
		if (
			frm.doc.trip_completed == 0 &&
			frm.doc.trip_status == "Pending" &&
			frm.doc.docstatus == 0
		) {
			frm.add_custom_button(
				__("Create Breakdown Entry"),
				function () {
					frappe.confirm("Do you want to create breakdown entry?", function () {
						frappe.call({
							method:
								"vsd_fleet_ms.vsd_fleet_ms.doctype.trips.trips.create_breakdown",
							args: {
		...
		if (
			!frm.doc.resumption_trip &&
			frm.doc.trip_status == "Breakdown" &&
			frm.doc.status != "Re-Assigned"
		) {
			frm.add_custom_button(
				__("Allocate New Vehicle Trip"),
				function () {
					frappe.call({
						method:
							"vsd_fleet_ms.vsd_fleet_ms.doctype.trips.trips.create_resumption_trip",
						args: {
							docname: frm.doc.name,
						},
						callback: function (r) {
	...
	setup: function (frm) {
		$(frm.wrapper).on("grid-row-render", function (e, grid_row) {
			if (grid_row.doc.request_status == "Requested") {
				$(grid_row.columns.request_status).css({
					"font-weight": "bold",
					color: "blue",
				});
	...
	reduce_stock: function (frm) {
		...
		frappe.call({
			method:
				"vsd_fleet_ms.vsd_fleet_ms.doctype.trips.trips.create_stock_out_entry",
			args: {
				doc: frm.doc,
	...
	route: function (frm) {
		frappe.model.with_doc("Trip Routes", frm.doc.route, function (frm) {
			var reference_route = frappe.model.get_doc(
				"Trip Routes",
				cur_frm.doc.route
			);
			...
			reference_route.fixed_expenses.forEach(function (row) {
				frappe.model.with_doc("Fixed Expenses", row.expense, function (frm) {
					var fixed_expense_doc = frappe.model.get_doc(
						"Fixed Expenses",
						row.expense
					);
	...
	date_of_departure_from_border: (frm) => {
		if (
			frm.doc.date_of_departure_from_border &&
			frm.doc.arrival_date_at_border
		) {
			var date1 = frappe.datetime.str_to_obj(
				cur_frm.doc.date_of_departure_from_border
			);
			var date2 = frappe.datetime.str_to_obj(
				cur_frm.doc.arrival_date_at_border
			);
	...
	view_on_map: function (frm, cdt, cdn) {
		if (locals[cdt][cdn].latitude & locals[cdt][cdn].longitude) {
			var url =
				...
				locals[cdt][cdn].latitude +
				"," +
				locals[cdt][cdn].longitude;
...
frappe.ui.form.on("Fuel Requests Table", {
	create_purchase_order: (frm, cdt, cdn) => {
		...
		frappe.call({
			method:
				"vsd_fleet_ms.vsd_fleet_ms.doctype.trips.trips.create_purchase_order",
			args: {
				request_doc: frm.doc,
				item: row,
			},
	...
	disburse_funds: function (frm, cdt, cdn) {
		...
		frappe.db
			...
			.then((r) => {
				const je_required =
					r.message &&
					typeof r.message.require_journal_entry_for_funds !== "undefined"
						? r.message.require_journal_entry_for_funds
						: 0;
				if (!je_required) {
					frappe.msgprint(
						__(
							"Automatic Journal Entry creation is disabled in Transport Settings. Skipping creation."
						)
					);
					return;
				}

				// proceed with creation when enabled
				frappe.call({
					method:
						"vsd_fleet_ms.vsd_fleet_ms.doctype.trips.trips.create_fund_jl",
					args: {
						doc: frm.doc,
						row: row,
					},
