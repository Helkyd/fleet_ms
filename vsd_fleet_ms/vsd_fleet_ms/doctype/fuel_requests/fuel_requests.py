...
def make_stock_entry(source_name, target_doc=None):
	#FIX 31-07-2026; Check if already has Issued!!!
	print ('Chegou make stock entry')
	fuel_requests_issued = frappe.db.sql(
		"""SELECT name, remarks FROM `tabStock Entry` WHERE remarks like %(remarks_)s""",
		{"remarks_": "%" + frappe.db.get_value("Fuel Requests", source_name, "name") + "%"},
		as_dict=1,
	)
	print ('tem ou nao ', fuel_requests_issued)
	if fuel_requests_issued == []:
		doc = get_mapped_doc(
			"Fuel Requests",
			source_name,
			{
