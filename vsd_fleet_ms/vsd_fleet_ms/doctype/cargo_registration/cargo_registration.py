...
def create_quotation(doc, rows):
	...
	if doc.docstatus == 0:
		...
		frappe.db.sql(""" UPDATE `tabCargo Detail` set quotation = %s where parent = %s and isnull(quotation) """, (invoice.name,doc.name), as_dict=False)
		frappe.db.commit()
		   
		
	frappe.msgprint(_("Quotation {0} Created").format(invoice.name), alert=True)
	return invoice